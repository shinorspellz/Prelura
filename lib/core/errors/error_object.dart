...
class ErrorObject {
  ...
  static ErrorObject mapFailures(Failure failure) => failure.when(
      ...
      cacheFailure: (message) => ErrorObject(title: 'Cache Failure', message: message ?? 'An error occured loading cached data'),
      requestFailure: (message, int code, [StackTrace? stackTrace]) => ErrorObject(title: 'Request Failure', message: message, code: code, stackTrace: stackTrace),
