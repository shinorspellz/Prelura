...
final filteredResultsProvider = Provider<List<PreluraCardModel>>((ref) {
  ...
  return mockData.where((model) {
    ...
    bool matchesFilters = filters.entries.every((filter) {
      ...
      final filterValue =
          filter.value; // This can be a single value or a List<String>
      ...
      if (filterValue is List<String>) {
        ...
        return filterValue.map((v) => v.toLowerCase()).any((filterVal) =>
            itemValue.toString().toLowerCase().contains(filterVal));
      }
